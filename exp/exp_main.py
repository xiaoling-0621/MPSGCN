...
class Exp_Main(Exp_Basic):
    ...
    def test(self, setting, test=0):
        ...
        print('test shape:', preds.shape, trues.shape)
        std_np = np.std(preds, axis=2)  # axis=2 是时间维度
        print("Std over time (numpy):", np.mean(std_np))
        cha = preds-trues
        cha_np = np.std(cha, axis=2)  # axis=2 是时间维度
        print("Std over time (numpy):", np.mean(cha_np))
